"""SQLite-based person registry with face embeddings and mood/score history."""
...
class PersonRegistry:
    """Stores known people (with face embeddings) and their mood + score history."""
    ...
    def log_mood(self, person_id, emotion, confidence=None, context=None,
                 eye_contact=None, volume=None, response_latency=None,
                 score=None, level=None):
        """Log a mood + score observation for a person."""
        self.conn.execute(
            """INSERT INTO mood_log
               (person_id, emotion, confidence, context,
                eye_contact, volume, response_latency, score, level, timestamp)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (person_id, emotion, confidence, context,
             eye_contact, volume, response_latency, score, level,
             datetime.now().isoformat())
        )
        self.conn.commit()

    def label_score(self, mood_id, label):
        """Caregiver labels a score reading (0-100) for training."""
        self.conn.execute(
            "UPDATE mood_log SET label = ? WHERE id = ?",
            (label, mood_id)
        )
        self.conn.commit()

    def get_training_data(self):
        """Query all labeled readings for model training."""
        cursor = self.conn.execute(
    ...
    def get_mood_history(self, person_id, limit=10):
        """Get recent mood + score history for a person."""
        cursor = self.conn.execute(
            """SELECT id, emotion, confidence, context,
                      eye_contact, volume, response_latency,
                      score, level, label, timestamp
               FROM mood_log WHERE person_id = ?
    ...
    def get_ruby_score_history(self, person_id, limit=50):
        """Get recent readings that have a Ruby Score."""
        cursor = self.conn.execute(
            """SELECT id, eye_contact, volume, response_latency,
                      score, level, label, timestamp
               FROM mood_log WHERE person_id = ? AND score IS NOT NULL
               ORDER BY timestamp DESC LIMIT ?""",
            (person_id, limit)
        )
        return [dict(row) for row in cursor.fetchall()]
