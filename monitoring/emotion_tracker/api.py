...
class RedRoverHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == "/api/beacon":
            self._handle_beacon()
        elif self.path == "/api/gesture/wave":
            self._handle_wave()
        else:
            self._send(404, {"error": "not found"})
    ...
    def _handle_wave(self):
        """Wave at Ruby and say hello from a caregiver."""
        ...
        try:
            ...
            if length > 0:
                ...
                sender_name = body.get("sender_name", "Mom")
        ...
        def _do_wave():
            from skills.wave_hello import _speak, _wave_arm
            import threading
            speak_t = threading.Thread(target=_speak, args=(greeting,))
            wave_t = threading.Thread(target=_wave_arm)
            speak_t.start()
            wave_t.start()
            speak_t.join(timeout=10)
            wave_t.join(timeout=10)

        threading.Thread(target=_do_wave, daemon=True).start()
        self._send(200, {"message": greeting, "status": "waving"})
