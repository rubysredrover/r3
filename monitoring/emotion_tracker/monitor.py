...
class EmotionMonitor:
    ...
    def _register_new_person(self, description, embedding, emotion, confidence, context):
        ...
        Voice output is handled by the agent — monitor stays silent.
        """
        name = "Unknown"
        ...
        print(f"[New person] Registered as '{name}' (ID: {person_id}) — agent will greet them")
        return person_id
    ...
    def _alert_mom(self, person_id, alert):
        ...
        # Voice alert handled by agent — monitor logs and updates API only
        try:
    ...
    def _on_mood_change(self, person, old_mood, new_emotion):
        ...
        if old_mood:
            ...
            if new_emotion in DISTRESS_EMOTIONS:
                print(f"[Distress] {name}: {new_emotion} — agent will respond")
            else:
